'use client'
// app/page.tsx — TheWall Main App
// Login → Smart Wallet (Alchemy Account Kit, gasless) → Dashboard
...
// ─── Constants ───────────────────────────────────────────────────────────────
const MAIN_WALLET = '0xba24d47e...f3f4e1'
...
export default function TheWall() {
  ...
  const fetchBalance = useCallback(async (address: string) => {
    try {
      ...
      setWalletData({
        ...ethData,
        solBalance: solData.solBalance || 0,
      })
    } catch (e) {
  ...
  // ── Login with email (Alchemy Account Kit simulation) ──
  const handleEmailLogin = async () => {
    if (!email.includes('@')) return
    setLoginStep('otp')
    // In production: call Alchemy Account Kit sendOtp(email)
  }
  ...
  const handleOtpVerify = async () => {
    ...
    setLoading(true)
    // Simulate smart wallet creation (replace with Alchemy Account Kit)
    await new Promise(r => setTimeout(r, 2000))
    const smartWallet: UserWallet = {
      address: MAIN_WALLET,
      type: 'smart',
      email,
    }
    ...
    return (
      <div className={styles.loginWrap}>
          ...
          {loginStep === 'email' && (
            <div className="fade-up-1">
              ...
              <input
                ...
                autoFocus
              />
              <button className={styles.btnPrimary} onClick={handleEmailLogin}>
                Send Code
              </button>
              <button className={styles.btnGhost} onClick={() => setLoginStep('home')}>
                ← Back
              </button>
            </div>
          )}
          ...
          {loginStep === 'otp' && (
            <div className="fade-up-1">
              ...
              <input
                ...
                autoFocus
              />
              <button className={styles.btnPrimary} onClick={handleOtpVerify}>
                Verify & Create Wallet
              </button>
              <button className={styles.btnGhost} onClick={() => setLoginStep('email')}>
                ← Back
              </button>
            </div>
          )}
        ...
        <div className={styles.loginFooter}>
          ⬡ THE WALL · DWIN · 2026 · KANNUR → DUBAI
        </div>
      </div>
    )
  }
    ...
    <div className={styles.dashWrap}>
      ...
      <header className={`${styles.header} fade-up`}>
        ...
        <div className={styles.headerRight}>
          ...
          <button className={styles.logoutBtn} onClick={() => setScreen('login')}>
            ⏻
          </button>
        </div>
      </header>
      ...
      <main className={styles.main}>
          ...
          <div className={styles.walletTop}>
            <div>
              <div className={styles.walletLabel}>
                {user?.type === 'smart' ? '⚡ SMART WALLET' : '👁 MAIN WALLET'}
              </div>
              ...
              {user?.email && (
                <div className={styles.walletEmail}>{user.email}</div>
              )}
            </div>
        ...
        <div className={`${styles.tabContent} fade-up-4`}>
                  ...
                  <div key={token.symbol} className={styles.tokenCard}>
                    ...
                    <div className={styles.tokenRight}>
                      <div className={styles.tokenPrice}>
                        {p ? `$${p.price.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : <span className={styles.loading}>$···</span>}
                      </div>
                      {p && (
                      ...
                      {bal > 0 && (
                        <div className={styles.tokenBal}>{bal.toFixed(4)} {token.symbol}</div>
                      )}
          ...
          {activeTab === 'prices' && (
            <div>
              {priceError && (
                <div className={styles.errorBanner}>⚠ Price feed unavailable. Add Alchemy API key.</div>
              )}
                ...
                {TOKENS.map(token => {
                  ...
                  return (
                    <div key={token.symbol} className={styles.priceRow}>
                      <span>
                        <span className={styles.tokenDotSmall} style={{ background: token.color }} />
                        {token.symbol}
                      </span>
                      <span className={styles.priceVal}>
                        {p ? `$${p.price.toLocaleString('en', { minimumFractionDigits: 2 })}` : '···'}
                      </span>
                      <span className={p && p.change24h >= 0 ? styles.green : styles.red}>
          ...
          {activeTab === 'history' && (
            <div className={styles.historySection}>
              ...
              <div className={styles.historyNote}>
                Connect Alchemy API key to load full transaction history
              </div>
            </div>
          )}
      ...
      <footer className={styles.footer}>
        ⬡ THE WALL · KANNUR → DUBAI · DWIN · 2026
      </footer>
    </div>
  )
}
